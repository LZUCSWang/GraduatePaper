...
\begin{algorithm}[h]
	\begin{algorithmic}[1]
		...
		\State $\mathcal{D}_{train} \leftarrow \{x_i\}_{i=1}^{epoch\_size}$ \Comment{Generate training instances}
		...
		\State $\theta \leftarrow \text{TrainBatch}(M_{\theta}, B_{model}, \mathcal{O}, \mathcal{B})$ \Comment{Algorithm 2}
		\EndFor
		...
		\State $\theta^* \leftarrow \theta$ \Comment{Save best model parameters}
...
\begin{algorithm}[h]
	\begin{algorithmic}[1]
		...
		\State $(C, \log p) \leftarrow M_{\theta}(x)$ \Comment{$C$: costs, $\log p$: log-probabilities}
